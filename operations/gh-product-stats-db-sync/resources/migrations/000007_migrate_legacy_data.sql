-- ============================================================================
-- PILOT Migration 000007: Migrate legacy data to new schema for a single day
...
-- (Previously run: October 2024 through 2026-07-11 fully migrated.)
...
WHERE DATE(r.created_at) = '2026-07-12'
...
WHERE ra.asset_name IS NOT NULL
  ...
  AND DATE(ra.created_at) = '2026-07-12'
ON DUPLICATE KEY UPDATE
...
-- data actually ends (the filter allows through 2026-07-12, but rows only
...
FROM repository_daily_snapshots
WHERE snapshot_date = '2026-07-12'
UNION ALL
SELECT 'release_asset_daily_snapshots', COUNT(*)
FROM release_asset_daily_snapshots
WHERE snapshot_date = '2026-07-12';

-- 4d. Per-day repo row count for the target date — a nonzero total here
-- confirms rows actually landed for this date (0 rows means the legacy cron
-- had already stopped running by this date, not an error).
