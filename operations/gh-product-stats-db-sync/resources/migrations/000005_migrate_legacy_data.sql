-- ============================================================================
-- PILOT Migration 000005: Migrate legacy data to new schema for a single day
-- Target Date: 2026-07-11 (final catch-up day — today.
-- CAUTION: if the new Ballerina sync cron has already started writing to this
-- database, this upserts over its rows for today — values should be
-- near-identical (both snapshot the same GitHub counters), but confirm the
-- new cron hasn't run for this date yet before running this.)
-- (Previously run: October 2024 through 2026-07-10 fully migrated.)
-- To reuse: update the date in ALL SEVEN filters below (Section 2 dedup +
-- outer, Section 3 dedup + outer, both 4c counts, 4d per-day counts) — a
-- partial edit silently migrates nothing while validation still looks
-- plausible.
-- ============================================================================
...
INNER JOIN (
    ...
    FROM repository
    WHERE DATE(created_at) = '2026-07-11'
    GROUP BY name, org_name, DATE(created_at)
) dedup
    ON dedup.max_id = r.id
WHERE DATE(r.created_at) = '2026-07-11'
...
WHERE ra.asset_name IS NOT NULL
  ...
  AND DATE(ra.created_at) = '2026-07-11'
ON DUPLICATE KEY UPDATE
...
-- data actually ends (the filter allows through 2026-07-11, but rows only
...
-- 4c. Verify exactly how many rows were inserted for the target date.
SELECT 'repository_daily_snapshots' AS tbl, COUNT(*) AS rows_migrated
FROM repository_daily_snapshots
WHERE snapshot_date = '2026-07-11'
UNION ALL
SELECT 'release_asset_daily_snapshots', COUNT(*)
FROM release_asset_daily_snapshots
WHERE snapshot_date = '2026-07-11';

-- 4d. Per-day repo row count for the target date — 0 rows means the legacy
-- cron had already stopped running by this date (not an error; it just tells
-- you legacy data ends before 2026-07-11).
